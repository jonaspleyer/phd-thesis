...
\epigraph{The purpose of abstracting is not to be vague, but to create a new semantic
...
Ein wichtiger Bestandteil wird auch die zugrundeliegende Denkweise, indem ich die
zellulären systeme in ihre "Aspecte" aufteile.
Diese werde ich im Einleitungskapitel davor motiviert habe.

%---------------------------------------------------------------------------------------------------
\section{Goals/Justification}
\begin{itemize}
    \item \cite{Miller2007} social science; also says that is important to consider the level of
        detail within the model
    \item develop most simple model (occam's razor) without "clutter"; existing tools do not have
        enough flexibility and often inherit many parameters
    \item general solutions like netlogo \todo[inline]{add netlogo citation} are good but not
        designed with cellular/biological applications in mind; therefore can often not leverage
        some fundamental assumptions for performance gain and other stuff such as topological
        cellular processes
    \item in general: most people want to look at large collections of agents, i.e. scale up the
        total number; few have attempted to stay at low cell numbers and dive deeper into cellular
        mechanics and behaviour; mindset like \acp{ca}: simple rules $\rightarrow$ collective
        behaviour
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{compile time hierarchy of encapsulation that mathces the domain model} \textbf{is
        bad}
\end{itemize}
...
\begin{itemize}
    \item interactions are local between cells; mostly blocked on long ranges by environment
    \item processes within environment are also mostly local: diffusion, fluid mechanics, even
        particle interactions (collisions etc.)
    \item most long-range interactions are slow and still inherently local: molecular signalling
        i.e. diffusion
    \item others are fast but still local
    \begin{enumerate}
        \item bioelectric waves in bacterial biofilms \cite{Prindle2015}
        \item mechanical signalling nitella chara \cite{Shimmen1997}
    \end{enumerate}
    \item only rare cases where interactions are of long range: "Galvanotaxis driven by
        Transepithelial Potential during Wound Healing" \cite{duBoisReymond1843,Zhao2006}
    \item we can leverage this to more efficiently perform numerical computations to obtain the
        dynamics of \ac{cabm} $\rightarrow$ domain decomposition
    \item split apart physical space and treat every subdomain as individual systems: get boundary
        conditions from adjoining (neighboring) subdomains
    \item other names for domain decomposition: FETI (Finite Element Tearing and Interconnecting),
        Spatial Partitioning, Mesh Partitioning
\end{itemize}

\textbf{Technicalities}
\begin{itemize}
    \item topological cellular processes are local more or less by definition
    \item split apart dynamics $\psi$
\end{itemize}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/03-cellular_raza/thread_scaling.png}
...
\label{section:scaling-with-problem-size}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/03-cellular_raza/thread_scaling.png}%
    \includegraphics[width=0.5\textwidth]{figures/03-cellular_raza/sim-size-scaling.pdf}
    \caption[Thread Scaling of cellular\_raza]
    {\textbf{TODO}}
    \label{fig:sim-size-scaling}
\end{figure}
