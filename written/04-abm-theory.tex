...
The concepts discussed in this section cover the concept of cellular identifiability, meaning the
ability to observe, model and asses cells as individual entitites which is foundational to the
individual-based approach of \acp{cabm}.
Treating every cell as individual entitity will allow our framework to properly account for
cell-to-cell variability and phenotypic/genotypic heterogeneity.
These concepts should not be confused with the question of cell types which is sometimes also
referred to as cellular identity.
In order to clearly separate these from each other, we first summarize the latter genomic approach
and then focus on the former individual-based choice.

\subsubsection{Differentiating Cell Types - The Success of Single-Cell Genomics}
The concept of cellular identity or the notion of cell types is to categorize individual cells
into groups of similar behaviour, but its precise meaning remains poorly
defined~\cite{Morris2019}.
When dealing with \acp{grn}, one can view any single cell as defined by its transcription factors
and resulting reaction dynamics~\cite{Cahan2014}.

Another viewpoint of cellular identity is by means of the epigenetic
landscape which was first introduced by Waddington in 1958~\cite{Waddington1958} and could later
be understood in terms of modern single-cell genomics~\cite{Huang2011,Wagner2016}.
Within this framework, cell-types can be understood as local attractors~\cite{Ye2018}.

In contrast to the \ac{grn}-based approach, other forms of cellular identity have also been proposed
over the later years~\cite{Kim2021}.
One can summarize all of the presented efforts by stating that their main goal is to categorize a
collection of cells by a respective commonality shared between them.

\subsubsection{Individual-based Approach \& Cellular Identifiability}
In contrast to the bulk-based approach explained just before, we are now interested in individual
cells of a larger collection.
This section discusses various conceptual and practical aspects of this individual-based approach.

\todo[inline]{rephrase the following sentences. Remove the word "identity". Make everything nicer.}
Leaving practical limitations aside, in principle cellular systems are characterized by the fact
that cells are tracable through time and space.
Moreover, this involves keeping track of their lineage which starts either at the initial time point
of the experiment or after the division of their mother cell.
The cell's lifetime ends when it reaches a final fate such as in death or division.
The experimental process of imaging cells and tracking their movement through time and space
is a considerable challenge to this day.
While a set of tools exist that facilitate static spatial
segmentation~\cite{Cutler2022,Stringer2020,Hardo2022}, the dynamic extension of imaging and tracking
cells through space still poses a substantial problem~\cite{Maka2014,Maka2023,Ulman2017,Wen2021}.

\todo{overall we need more citations here}
These considerations lead to a notion of cellular identity which is not coupled to the physical
properties of the cell but rather defined by its lineage.
To illustrate the practial implications of this idea, we perform a thought experimet.
We assume that a cell is defined by its position on a lattice grid and discrete internal states.
...
particles which is employed across statistical physics.
Quantum systems in particular are known to utilize this assumption for elementary particles such as
bosonic ones where exchange of two particles leaves the wave function unchanged.
However, said assumption is in general not valid for such complex entities as biological cells.
The reason that these cells seem indistinguishable is that our reduced model captures only
...
it is not the only source of identity challenges.
...
We set out to link experimental cells to the model output but assume that the chosen metric only
involves physical cellular properties such as the position of the cell or some form of readout
e.g. by tagging with \ac{gfp}~\cite{Chalfie1994}.
Consider the case where two cells $c_{m,1},c_{m,2}$ in the model output are approximately equally
close to the experimental state of a particular cell $c_e$ under this chosen metric, i.e.
...
Furthermore even if the obtained distance values would exhibit a clear discrepancy, the link between
model and experimental reality could be flawed if simply the closest cell is also assumed to be
linked to the experimental one.
...
would not be caught.
This will not only result in a mismatched link between but more importantly to a lower
measured discrepancy between model and reality.
Such a behaviour is unacpetable when it comes to estimating parameters of any system and therefore
shows the clear need to include this cellular identity.
...
In a final thought experiment we omit any spatial considerations or concepts of closeness in order
to further exemplify the idea of cellular identity.
...
All of these thought experiments would be solved by keeping track of the cellular identity in the
form of a identifier which is not only unique at any given time point but throughout the whole
...
These identifiers can not only be used to distinguish between agents but also to keep track of
mother-daughter relationships.
The idea of cellular identity being closely tied to their lineage is further made clear when listing
cellular relations as a family tree.
...
\subsection{Construction of the Cellular State}
...
\subsubsection{A Primer on Cellular Reproduction \& Death}
...
death)~\cite{BOHM2003,Green2010,Raff1998}, autophagy~\cite{Licheva2025,Galluzzi2017} or
