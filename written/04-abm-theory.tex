...
\paragraph{Other processes} such as cellular fusion~\cite{Ogle2004,Whitlock2024} where multiple
cells are combined into one single new cell can also alter the cardinality of the cellular state.
This phenomenon was first hinted at by Theodor Schwann~\cite{Schwann1847} as early as 1847.
It would however take until 1960 before this was first induced experimentally~\cite{Barski1960}.
Fusing two or more cells, regardless of their type, can be viewed as removing said individual
entities and inserting a completely new agent as displayed in Figure~\ref{fig:theory-cell-fusion}.
...
\begin{figure}[h]
    ...
    \caption[Cellular Fusion]{
        The two cells $c_1$ and $c_2$ which fuse to form a completely new cellular entitiy $c_3$.
    }
    \label{fig:theory-cell-fusion}
\end{figure}

However when modeling such systems, it is important to also consider other forms of cell addition
and removal.
Most computational studies are only able to capture a slice of the actual biological reality in
...
This is exemplified by microfluidic devices such as mother machines in which bacteria can be kept in
a state of continuous growth while excess cells are simply sucked away by fluidic forces.
\todo{citation}
One technique to avoid the challenges of inspecting a subslice of a larger biological system
is to consider periodic spatial boundary conditions within the constructed model.
This is for instance commonly employed in densely packed epithelial cell sheets but can not be
applied generally across all systems as the previous example of the mother machine shows.
\todo{citation}
...
\todo[inline]{rewrite this sentence such that it makes more sense here}
...
meiosis~\cite{Urry2020} into separate daughter cells.
