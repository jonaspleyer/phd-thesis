...
We perform yet another thought experiment.
...
Consider an initial state of two cells $c_1\in A,c_2\in B$ of distinct species and a final state of
three cells of species $c_3\in A,c_4\in B,c_5\in B$.
Considering the dynamics we have two possible paths of achieving the final state.
Firstly, cell $c_2$ can divide, resulting in $c_4$ and $c_5$ while $c_1=c_3$ stays the same.
As an alternative $c_1$ can divide into $c_3$ and $c_4$ and the daughter cell $c_4$ differentiates
into species $B$ while $c_2=c_5$ stays identical.
The initial and final state are identical in both scenarios but the biological interpretation is
changed completely.
Without knowing the de-facto parental relationship between cells $c_1,c_2$ and $c_3,c_4,c_5$, we can
give no further insights into these dynamics thus missing out on the crucial biological
interpretation if differentiation of species $A$ or division of species $B$ was the driving factor.
...
\begin{figure}
    ...
    \begin{tikzpicture}[level distance=1cm,
        ...
        \node[] at (-2.5,0) {Initial};
        \node[] at (-2.5,-1) {Final};

        % 1st Process
        \node[] at (-1.1,0.6) {\textbf{A}};
        \draw (-1.5,1) rectangle (3,-2);
        \node[circle, draw, fill=blue!15] (c1) {$c_1$}
        child {node[circle, draw, fill=blue!15] (c3) {$c_{3}$}}
        child {node[circle, draw, fill=green!15] (c4) {$c_{4}$}};
        \node[circle, draw, fill=green!15] at (2,0) {$c_2$}
        child {node[circle, draw, fill=green!15] (c5) {$c_{5}$}};

        % not equivalent
        \node[] at (4,-0.5) {$\nLeftrightarrow$};

        % 2nd Process
        \node[] at (5.4,0.6) {\textbf{B}};
        \draw (5,1) rectangle (9.5,-2);
        \node[circle, draw, fill=blue!15] at (6,0) {$c_1$}
        child {node[circle, draw, fill=blue!15] (d3) {$c_{3}$}};
        \node[circle, draw, fill=green!15] at (8,0) {$c_2$}
        child {node[circle, draw, fill=green!15] (d4) {$c_{5}$}}
        child {node[circle, draw, fill=green!15] (d5) {$c_{5}$}};
    \end{tikzpicture}
    \caption[]{
        Two systems with identical initial and final state can be achieved through distinct cellular
        processes.
        (A) Cell $c_1$ of species $A$ divides into $c_3$ and $c_4$ where the latter differentiates
        into species $B$.
        Meanwhile cell $c_2$ stays the same and is thus identical to $c_5$.
        (B) Cell $c_2$ divides into $c_5$ and $c_6$ while $c_1$ stays the same.
        \todo[inline]{short caption}
    }
    \label{fig:theory-thought-experiment-2}
\end{figure}

The idea of cellular identity being defined by their lineage is further made clear when listing
cellular relations as a family tree.
Figure~\ref{fig:theory-thought-experiment-3} shows a hypothetical change between two child-cells.
...
considerations.
They are also crucial for our comparisons of \acp{ca} to \acp{abm}.
