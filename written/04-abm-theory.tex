...
\paragraph{Other forms of additional and removal} need to be accounted for as well.
...
With the previous considerations in mind, we now focus on the instantanous event in which the
discrete entity of a single cell is replaced/removed or inserted.
With this framework, cell division can viewed as the combination of the removal of one (mother) cell
and the insertion of new daughter cells while cell death is simply the removal of one cell.
In practical cases, it is also important to include the concept of insertion and removal of cells
which have not been the result of cellular division or death.
Such a situation can become relevant, i.e., when observing a spatial subslice of a larger system
which is entered by an external cell.
In this case, a new unique identifier $\iota$ must be picked and assigned to this cell but no known
relation to any other existing cell can be assumed.
Analogously, bacteria growing within the confinements of a mother machine may be sucked away by a
fluidic flow, thus resulting in a de-facto removal from the observable region.

We will now formalize these ideas by defining how one cellular state $(I_p,M)$ is mapped onto
another by various cellular processes.

First, we consider the division of a single cell and then extend its definition to multiple cells.
As mentioned earlier, the combination of multiple division events are assumed to ocurr at the same
time point.
Any successive division events can simply be described by iteratively applying these methods.

As noted earlier, most cells divide and produce exactly two new daughter cells but other forms such
as multiple fission~\cite{Suresh1994,Peckov1990} or budding of yeast~\cite{Feldmann2010} can be
observed in reality.
Thus our single-cell division map $\gamma$ should be able to cover cases where more than two
daughter cells result from a single event and also be able to retain the mother cell.
This means that a single cell $c\in C$ is mapped to a tuple of arbitrary length $(c_1,c_2,\dots)$
which live in a space $C^n$.
To enforce the distinction between the products $C^n$ where $n\geq 2$ we utilize the disjoint union
$\sqcup_{n\geq 2}C^n$ as the target space of the division process.
% which can be formalized by $\sqcup_{n\geq 2}C^n=\{(n,\vec{c})|n\geq 2,\vec{c}\in C^n\}$.
We therefore arrive at the function signature
\begin{equation}
    \gamma:C\rightarrow \sqcup_{n\geq 2}C^n.
\end{equation}
...
manage administrative tasks such as the creation and removal of unique new identifiers for the
daughter cells.
Therefore, $\Upsilon$ can be defined abstractly without the particular knowledge of $\gamma$ and is
specific to the respective simulation framework.

\begin{definition}[Single Cell Division]
    We call a map $\gamma:C\rightarrow \sqcup_{n\geq 2}C^n$ a single division function and
    $\Upsilon:\{\gamma:C\rightarrow \sqcup_{n\geq 2}C^n\}\times I\times\CS\rightarrow\CS$ a
    single division operator if for any initial state $(I_p,M)\in\CS$ with $c_\iota\in M$ indicating
    the mother cell and final state $(J_p,N)=\Upsilon(\gamma,\iota,(I_p,M))$, the following
    conditions hold:
    \begin{enumerate}
        \item (Removal) Exactly one cell is removed $M\backslash N=\{c_\iota\}$
        \item (Insertion) At least two new cells are created by the division function $\gamma$
        \begin{align}
            \gamma(c_\iota)&=(c_1,\dots,c_n)\\
            N\backslash M&=\{c_{i_1},\dots,c_{i_n}\}
        \end{align}
        where $\#N\backslash M\geq2$.
...
From an implementation viewpoint, it is desirable to only specify the single division map $\gamma$
which maps a single cell $c$ onto new daughter cells.
The method $\Upsilon$ which acts on $\CS$ may then be defined by the underlying modeling framework.
It will depend on the choice of identifiers $\iota\in I$ but the overall dynamics of the system
should be invariant under its particular choice.

Definition~\ref{definition:cell-division-single} maps a single cell $c\in C$ onto multiple new cells
$\{c_i\}\subset\cup_{n\geq 2}C^n$

The function signature $\gamma:C\rightarrow\cup_{n\geq2}C^n$
