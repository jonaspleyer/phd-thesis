...
This shows that the presented ideas are relevant to some applications.
...
We have now defined a structure which can capture various snapshots of unique cellular agents but
are still missing any notion of dynamics.
While processes such as movement of the cells or physical interactions can change the individual
cellular configurations $c$, they preserve the unique cells $\pi_1(M)$ and $I_p$ which are contained
within the state.
To continue our considerations of uniqueness, we now want to turn our attention to processes such as
cell division and cell death that lead to changes in the cellular state which require updating the
tracked indices.
Other dynamical processes are discussed later (see \todo{insert ref}).
\todo{maybe hint that the rest is easy and comes later}
...
daughter cells.
On the subcellular level, these processes ocurr over extended periods of time which can be
captured by a corresponding dynamical description of the process leading up to the time point in
question (see section~\ref{section:dynamics}).
Our \ac{abm} approach represents cells as discrete entities with well-defined division and death
timings which may well be one of its weaknesses.
In animal cells the time of division can for instance be defined by the end of cytokinesis~\cite{}
\todo{citation}
which is the instant when the microtubule and membrane that form the bridge between two daughter
cells is cut.
However it was shown that human HeLa and mouse BALB/c 3T3 cells are already in $G_1$ state at this
time point~\cite{Gershony2014}.
As expressed in the introductory section (\todo{ref. introduction} \ref{}), \acp{abm} usually take a
mesoscopic approach and it is therefore the task of any researcher to pick a notion of division time
that is appropriate for their desired model.
...
Omitting the preceding dynamics, with this framework, cell division can viewed as the combination
of the removal of one (mother) cell and the insertion of new daughter cells while cell death is
simply the removal of one cell.
