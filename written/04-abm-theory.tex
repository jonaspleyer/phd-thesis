...
is irrelevant as long as they adhere to the discussed principles.
...
to enforce in other systems such as when high stochastic fluctuations lead to decorrelation of any
two successive measured time points.
In these scenarios it is questionable if the concept of cellular identity does provide
meaningful insights into the spatial dynamics of the system.
But it can still yield valuable information about cellular fate through differentiation.
...
\paragraph{Internal budding \& multiple fission} are other forms of reproduction which are similar
to the previous case but do not produce two cells but rather a variable amount.
Internal budding is observed in parasites such as \textit{Toxoplasma gondii}~\cite{Smyth1994} and
multiple fission in some \textit{Myxogastria}~\cite{Baldauf1997}, \textit{Blastocystis
hominis}~\cite{Suresh1994} and \textit{Trypanoplasma}~\cite{Peckov1990}.
For these less common cases, Figure~\ref{fig:theory-division-basic} must be adjusted to produce an
arbitrary amount of cells $n$, not just $n=2$.

% \cite{Niki2014} - S. japonicus alone is dimorphic yeast, which can transit from unicellular yeast to long filamentous hyphae.

\paragraph{Asymmetric reproduction} can, i.e., be observed in \textit{Saccharomyces cerevisiae},
also known as \textit{budding yeast}.
It is an eukaryotic model organism for molecular cell biology which reproduces asymmetrically, by
forming a bud on the envelope of the mother cell that separates into a new daugher
cell~\cite{Feldmann2010}
