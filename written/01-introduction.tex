...
\epigraph{
    Auf analoge, jedoch sehr mannigfaltige Weise bilden sich die Elementartheile
    aller Gewebe aus Zellen, so dass man den Grundsatz aufstellen kann: Dass es ein gemeinsames
    Entwicklungsprinzip für die verschiedensten Elementartheile der Organismen gibt, und dass die
    Zellenbildung dieses Entwicklungsprinzip ist.\\
    \vspace{0.25cm}
    The elementary parts of all tissues are formed of cells in an analogous, though very diversified
    manner, so that it may be asserted, that there is one universal principle of development for
    the elementary part of organisms, however different, and that this principle is the formation
    of cells.
}{\textit{Theodor Schwann (1839, translated by Henry Smith)}}
...
\begin{itemize}
    ...
    \item \todo{Look at this paper in more detail \cite{Lomas2014}. It is very very cool!}
\end{itemize}

\subsection{The Cellular Identity - Group or Individuum?}
This section discusses the concept of cellular uniqueness from a practical and philosophical
perspective.
It is the assumption that each cell, even if genetically identical to each other is intrinsically
distinct to each other and fundamental to the individual-based approach which \acp{cabm} utilize.
This allows our framework to properly account for cell-to-cell variability and phenotypic/genotypic
heterogeneity.
These concepts should not be confused with the categorization of cells into distinct types which is
also referred to as "cellular identity".
In order to clearly separate them from each other, we first summarize the latter genomic approach
and then focus on the former individual-based choice.

\subsubsection{Classification of Cell Types - The Success of Single-Cell Genomics}
The overarching goal of the concept behind cellular identity, or classification of "cell types", is
to group cells by shared behaviour although its precise meaning remains poorly
defined~\cite{Morris2019}.
One way to define cellular identity is by means of \acp{grn} which capture the cells active
transcription factors and resulting gene expression profiles~\cite{Cahan2014}.
Another approach is to utilize the epigenetic landscape first envisioned by Waddington in
1957~\cite{Waddington1958} and has since been understood in terms of modern single-cell
genomics~\cite{Huang2011,Wagner2016}.
Within this framework, cell-types can be understood as local attractors~\cite{Ye2018}.
Ultimately, these approaches share a common goal: classifying cell populations based on shared
characteristics.

\subsubsection{Cellular Uniqueness - An Individual-based Approach}
The concept of cellular uniqueness refers to the fact that no two cells are identical, even if they
share the same genetic material.
This observation can be the result of stochastic noise~\cite{Swain2002,Raj2008} and phenotypic
adaptation to external influences~\cite{WestEberhard1989,Price2003}.
Given the underlying complexity and sheer amount of possible combinations will in practice never
lead to two identical cells, even in tightly controlled environments~\cite{Spudich1976,Lestas2010}.
For our mathematical considerations we will therefore always keep in mind that the parameters which
control the dynamics of our system are in principle intrinsic to the individual cells/agents.
...
\begin{itemize}
    ...
    \item another example: HeLa Cells (very important for cancer; no spatial model yet)
    ...
    \item "All models are wrong but some are useful"
        ...
        He essentially says that simpler models can yield meaningful results if applied judiciously.
        But how can we construct simpler \acp{abm}? Only if we are flexible! Otherwise we are stuck.
\end{itemize}
...
\section{\acl{abm} as the basis for World Models}
\begin{itemize}
    \item world models need to be: predictive, causal, abstract, updatable. \acp{cabm} tick all
        boxes
\end{itemize}
...
\begin{sidewaystable}
    ...
    \todo[inline]{insert corresponding supplementary table}
    \todo[inline]{final fix: ensure that table is rotated outwards (depending
    on even/odd pagenumber)}
